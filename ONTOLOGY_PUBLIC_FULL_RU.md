...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
AD8. DERIVATION OF THE SPEED OF LIGHT AS A GEOMETRIC INVARIANT

This section derives the dimensionless speed of light from the
structural invariants of the field.  No external parameters,
calibration to SI units, or fitting to other constants are used.
All initial quantities are obtained from the halving operator H,
the E8 root lattice, and the Cube27 fractal geometry earlier in
the document.  The result is a pure number that characterises the
geometry of space and time; its expression in metres per second is
a separate step of unit calibration, not part of the invariant
itself.

8.0. Structural Invariants (and Their Origin)

  The derivation rests on three numbers.  None is a free choice;
  each follows from the geometry already developed.

  – Tick period = 2.
    Time is generated by the halving operator H through the
    sequence T → T⁻ → T.  Theorem H1 (Section 6) proves that the
    natural period of this cycle is exactly 2.  A period of 1
    would erase the distinction between successive states and
    remove time; 2 is the smallest non‑trivial period for a system
    that alternates between two configurations.

  – Fractal depth = 128.
    The E8 root system contains exactly 128 spinor roots — vectors
    with coordinates ±1 and an even number of minus signs (Section
    47).  This number equals 2⁷, where 7 is the number of imaginary
    octonionic directions.  The 128 spinors are the full set of
    binary distinctions available in the imaginary unit space; they
    therefore fix the number of nested levels in the Cube27 fractal
    at exactly 128.  A smaller depth would leave unused binary
    degrees of freedom; a larger depth would require more imaginary
    units than the octonions possess.  Over 128 levels each level
    multiplies the linear scale by 3, giving the overall scale
    factor

        λ = 3¹²⁸ = 9⁶⁴.                                          (1)

    Independently, the same scale factor appears in the mass
    spectrum: the exponential dependence of quark and lepton masses
    on the generation number (Section 58) involves the ratio 3⁶⁴,
    derived from the Dirac operator on S⁷ and the G₂ structure.
    The appearance of the same number in both the spatial and the
    mass sectors is a cross‑validation confirming that the fractal
    depth 128 is a genuine invariant of the theory.

  – Penetration ratio = 1/9.
    In one Cube27 subdivision a cube is split into 3×3×3 = 27
    subcubes.  Exactly three of them lie on the spatial diagonal
    (x = y = z), so the fraction of space on the axis is
    3/27 = 1/9 (Section 70.2).  This ratio is a geometric fact,
    not a parameter.

  From these three invariants the cosmological constant Λ is
  obtained in Section 70 as the unique combination that has the
  correct dimension 1/(length)² and respects the Pythagorean
  identity of the field:

      Λ = (tick period)² · (penetration ratio)^(fractal depth)
        = 2² · (1/9)¹²⁸ = 4 · (1/9)¹²⁸ .                        (2)

  The derivation of (2) from H and the Cube27/E8 geometry is
  given in Section 70; the result is used here as an established
  starting point.

8.1. Space and Time in the Field

Space is a 3‑sphere S³ of radius R.  Time is discrete, with the
tick period fixed to 2.  For a 3‑sphere the cosmological constant
satisfies Λ = 1 / R²; therefore

    R = 1 / √Λ = 1 / √[ 4 · (1/9)¹²⁸ ] = (1/2) · 9⁶⁴ .        (3)

The characteristic speed of the deepest fractal level is defined
by the scale factor λ and the tick period:

    v₀ = λ / tick = 9⁶⁴ / 2 .                                   (4)

8.2. Maximum Propagation Speed

On a 3‑sphere the longest path that does not reverse direction is
half of a great circle; that distance is πR.  The causality
structure of the field permits information to propagate one such
step per tick.  Hence the speed of light in absolute field units
(length per tick) is

    c = (πR) / tick = (π/2) · R = (π/2) · (1/2) · 9⁶⁴
      = (π/4) · 9⁶⁴ .                                           (5)

8.3. Dimensionless Invariant

The ratio of the speed of light to the characteristic speed v₀
is

    c / v₀ = [ (π/4) · 9⁶⁴ ] / [ 9⁶⁴ / 2 ] = π/2 .              (6)

All powers of 9 cancel; the result depends only on the geometric
constant π and on the factor 1/2 that originates from the relation
between R and the fractal scale λ.  This ratio is a dimensionless
invariant: it does not depend on the choice of length or time
units, nor on the absolute size of the universe.  It characterises
the geometry of space and time themselves.

Thus the dimensionless speed of light in the natural units of the
Opterium framework is exactly π/2.
...
The derivation uses only the three structural integers listed in
8.0.  None of them was fitted to any observable.  Their combination
yields Λ, R, v₀, and finally the dimensionless invariant π/2.  The
same scale factor 9⁶⁴ appears independently in the mass spectrum
