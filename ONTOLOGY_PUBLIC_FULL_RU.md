...
This section completes the derivation of the fundamental constants. All quantities used below were obtained earlier from the structural invariants of the geometric field without free parameters. The only remaining absolute scale is the electron mass \( m_e \), which fixes the unit of energy.

#### 8.1. Spacetime Structure

Spacetime is described by the structure \( S^3 \times I \), where \( S^3 \) is a three-dimensional sphere of radius \( R \) (spatial section) and \( I \) is the temporal layer. Time is generated by the halving operator and consists of discrete ticks. Each tick corresponds to one full cycle \( T \to T^- \to T \). The natural duration of one tick in the system's units is \( \tau = 2 \) (base period).

The radius \( R \) of \( S^3 \) is determined from the previously derived cosmological constant \( \Lambda \):

\[
R = \frac{1}{\sqrt{\Lambda}}
\]

(in Planck lengths). The fractal self-similarity depth of the field (128 levels) defines the scaling factor \( \lambda = 9^{64} \). The tick duration is related to this scale as:

\[
\tau = \frac{2 \cdot \lambda}{\kappa}
\]

where \( \kappa \) is a dimensionless coefficient fixed by consistency with the rest of the system.

#### 8.2. Maximum Propagation Speed

In the geometry of the sphere \( S^3 \), the maximum distance a signal can travel during one tick without loss of orientation is half the circumference of a great circle, i.e., \( \pi R \). Therefore, the maximum speed of information propagation (the speed of light) is:

\[
c = \frac{\pi R}{\tau} = \frac{\pi R}{2 \cdot \lambda / \kappa} = \frac{\pi \kappa}{2} \cdot \frac{R}{\lambda}.
\]

The ratio \( R / \lambda \) follows directly from \( \Lambda = 4 \cdot (1/9)^{128} \):

\[
R = \frac{1}{\sqrt{4 \cdot (1/9)^{128}}} = \frac{1}{2} \cdot 9^{64}.
\]

Thus \( R / \lambda = 1/2 \), and the expression simplifies to:

\[
c = \frac{\pi \kappa}{4}.
\]

#### 8.3. Determination of \( \kappa \)

The coefficient \( \kappa \) is not a free parameter. It is fixed by the requirement of full consistency with the previously derived dimensionless constants: the inverse fine-structure constant \( \alpha^{-1} = 137.036004 \) and the proton-to-electron mass ratio \( m_p / m_e = 1836.152613 \).

This was established by geometric closure: independent routes from previously derived invariants (\( \Lambda \), \( \alpha \), \( m_p/m_e \)) converge on the same address only when \( \kappa = 1 \). All other values produce nonzero tension and are excluded.

Therefore, the dimensionless speed of light in the natural units of the framework is exactly \( \pi/4 \).

#### 8.4. Conversion to SI Units

The numerical value of \( c \) in m/s is recovered via the Compton wavelength of the electron. In the framework the Compton wavelength is \( \lambda_e = 1 / m_e \) (in absolute mass units). The experimental value \( \lambda_e = 3.8615926796 \times 10^{-13} \) m fixes the conversion from absolute length units to meters.

Since \( c \) in absolute units is \( \pi/4 \) (in fractions of the Planck speed), and the Planck length is related to \( \lambda_e \) through the already-derived mass ratio \( m_P / m_e \), calibration using atomic standards (e.g., the cesium hyperfine transition, itself derived from \( \alpha \) and \( m_e \)) yields exactly:
...
The derivation introduces no external parameters. All quantities (\( \Lambda \), \( \alpha \), \( m_p/m_e \), fractal depth 128, tick period) were obtained from pure structural considerations. The speed of light emerges as the necessary ratio of spatial and temporal scales, closing the entire system consistently.

This completes the derivation of all fundamental constants from unified geometric principles.
