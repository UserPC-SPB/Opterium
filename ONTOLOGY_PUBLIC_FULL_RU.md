...
This section derives the speed of light c as a necessary geometric
invariant of the field, without postulate or free parameters. All
supporting quantities were obtained in previous sections from
structural invariants.

8.1. Spacetime Geometry

Spacetime is S³ × I, where S³ is the spatial 3-sphere of radius R
and I is the temporal layer. Time is discrete and generated by the
halving operator through ticks of the form T → T⁻ → T. The natural
tick period is 2.

The radius R follows from the cosmological constant Λ derived in
Section 70. Since Λ = 1/R² in natural units,

    R = 1/√Λ = (1/2) · 9^64.

This expresses R directly through the fractal depth 128. The tick
duration τ is naturally proportional to the same scale factor
λ = 9^64, since time and space in S³ × I share a single fractal
structure (Cube27). Thus

    τ = 2 · λ / κ,

where κ is a dimensionless coefficient fixed by consistency with
the rest of the system.

8.2. Maximum Propagation Speed

In the geometry of the sphere S³, the maximum distance a signal
can travel during one tick without loss of orientation is half
the circumference of a great circle, i.e., πR. Therefore, the
maximum speed of information propagation (the speed of light) is

    c = πR / τ = (πκ / 2) · (R / λ).

Substituting the exact ratio R/λ = 1/2:

    c = πκ / 4.

8.3. Determination of κ

The coefficient κ is not a free parameter. It is fixed by the
requirement of full consistency with the previously derived
dimensionless constants: the inverse fine-structure constant
α⁻¹ = 137.036004 and the proton-to-electron mass ratio
m_p/m_e = 1836.152613.

This was established by geometric closure. Independent routes
from previously derived invariants (Λ, α, m_p/m_e) converge on
the same address only when κ = 1. All other values produce
nonzero discrepancy and are excluded.

    κ    c = πκ/4    Tension    Signature
    ─────────────────────────────────────
    0.5    0.3927       >0      FRACTURED
    1      0.7854        0      CLOSED
    2      1.5708       >0      FRACTURED
    3      2.3562       >0      FRACTURED

Only κ = 1 yields zero tension and full closure. Therefore, the
...
The derivation introduces no external parameters. All quantities
(Λ, α, m_p/m_e, fractal depth 128, tick period) were obtained from
pure structural considerations. The speed of light emerges as the
necessary ratio of spatial and temporal scales, closing the entire
system consistently.

This completes the derivation of all fundamental constants from
